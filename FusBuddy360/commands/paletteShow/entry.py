...
CMD_ID = f'{config.COMPANY_NAME}_{config.ADDIN_NAME}_PaletteShow'
CMD_NAME = 'FusBuddy360'
CMD_Description = 'Open FusBuddy360 chat'
PALETTE_NAME = 'FusBuddy360'
# Show this command in the main toolbar
IS_PROMOTED = True
...
def palette_incoming(html_args: adsk.core.HTMLEventArgs):
    ...
    if html_args.data:
        try:
            ...
            message_data = {"_raw": html_args.data}
    ...
    # Handle chat messages from the palette.
    if message_action == 'userQuery':
        user_text = (message_data.get('text') or '').strip()
        if not user_text:
            html_args.returnData = 'Ignored empty message'
            return

        # Capture read-only context snapshot.
        ctx = context_capture.capture_context()

        # Ask the coach to generate a structured learning response (Step 5).
        result = coach.generate_response(user_text, ctx)
        futil.log(f"[FusBuddy360] result: {result}")
        # reply_text = result.get('text', '').strip() or "Sorry, I couldn’t generate a response."
        reply_text = "Sorry, I couldn’t generate a response."
        if result and result.get('text', ''):
            reply_text = result.get('text', '').strip() 

        palette = adsk.core.Palette.cast(html_args.firingEvent.sender)
        if palette:
            palette.sendInfoToHTML('contextUpdate', json.dumps(ctx))
            palette.sendInfoToHTML('assistantMessage', json.dumps({'text': reply_text}))

    # Handle saving an API key from the settings UI.
    elif message_action == 'saveApiKey':
        api_key = (message_data.get('apiKey') or '').strip()
        provider = (message_data.get('provider') or 'openai').lower()
        
        if not api_key:
            html_args.returnData = 'No API key provided'
            return

        # Do not log the actual key; just log that we updated it.
        provider_name = 'OpenAI' if provider == 'openai' else 'Gemini'
        futil.log(f'{CMD_NAME}: Received request to update {provider_name} API key (length={len(api_key)})')
        try:
            if provider == 'openai':
                user_config.set_openai_api_key(api_key)
            elif provider == 'gemini':
                user_config.set_gemini_api_key(api_key)
            else:
                html_args.returnData = f'Unknown provider: {provider}'
                return
            html_args.returnData = f'{provider_name} API key saved for this user.'
        except Exception as ex:
            futil.log(f'{CMD_NAME}: Failed to save {provider_name} API key: {ex}', adsk.core.LogLevels.ErrorLogLevel)
            html_args.returnData = f'Failed to save {provider_name} API key. See Text Commands for details.'
